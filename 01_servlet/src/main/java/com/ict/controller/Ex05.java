...
public class Ex05 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 요청에 맞는 일처리를 하는 곳으로 전달하고
		// 해당 일처리의 결과를 받아서 클라이언트에게 응답하는 역할을 한다.
		...
		// 이렇게 클라이언트가 요청을 처리하기 위해서 함께 전달되는 정보를 '파라미터'라고 한다.
		
		// * 파라미터의 자료형은 기본적으로는 String 이거나 String[] 이다.
		// get 방식 : 주소창에 파라미터 정보가 보인다.(?name=value&name=value&name=value&....)
		// post 방식 : 주소창에 파라미터 정보가 보이지 않는다.
		
		// 파라미터를 받아서 처리하는 메서드들 (name만쓰면 알아서 값이 불러와짐)
		...
		// 만약에 아이디와 패스워드가 파라미터 값으로 넘어왔을때 처리하는 방법
