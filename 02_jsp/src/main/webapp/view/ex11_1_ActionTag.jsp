...
<%--
	...
	사용법 : <jsp: 액션태그>
	 ...
	 1. include : 현재 페이지에서 다른 페이지를 호출 또는
	 			  다른 페이지의 결과를 현제 페이지에서 보여주는 것
	 			  
	 	- include 지시어 : <%@ include file="가져올 페이지"%>
	 	  다른 페이지 소스 전체를 가져와서 현제 페이지에서 한번에 처리
	 	  파라미터 값을 전달 할 수 없다.
	 	  잘 변경되지 않는 정적인 페이지에 많이 사용된다.
	 	  
	 	- inclue 액션태그 : <jsp:include page="가져올 페이지"></jsp:include>
	 		제어권을 해당 페이지에 넘겨서 일처리를 한 후 결과만 가져온다.
	 		파라미터값을 전달 할 수 있다.(param 액션 태그를 사용)
